...
def test_cliente_mora_activa(db_conn):
    """
    Escenario 2: Cliente con mora activa entre 7 y 10 días -> Categoría C (Congelamiento de cupo).
    """
    cursor = db_conn.cursor()
    # Cliente con saldo pendiente
    cursor.execute("""
        INSERT INTO clientes (nombre, limite_credito, saldo_actual, score_crediticio, categoria_riesgo, nivel_vinculo)
        VALUES ('Cliente Con Mora 8 Días', 1000.0, 400.0, 50, 'C', 'conocido_referido')
    """)
    cliente_id = cursor.lastrowid
...
def test_cliente_cartera_critica(db_conn):
    """
    Escenario 3: Cliente con mora > 10 días -> Categoría D (Bloqueo automático / Alerta Roja).
    """
    cursor = db_conn.cursor()
    cursor.execute("""
        INSERT INTO clientes (nombre, limite_credito, saldo_actual, score_crediticio, categoria_riesgo, nivel_vinculo)
    ...
    # Cargo pendiente desde hace 15 días (> 10 días de mora)
    cursor.execute("""
        INSERT INTO cuentas_por_cobrar (cliente_id, tipo_movimiento, monto, saldo_resultante, fecha_movimiento)
        VALUES (?, 'cargo', 800.0, 800.0, datetime('now', '-15 days'))
    """, (cliente_id,))
...
def test_cliente_cold_start(db_conn):
    """
    Escenario 4: Cliente nuevo sin historial previo -> Protocolo Cold-Start.
    Evalúa SW3 y asigna cupo semilla ($30.000 a $50.000 COP) a 15 días.
    """
    cursor = db_conn.cursor()
    cursor.execute("""
        INSERT INTO clientes (nombre, limite_credito, saldo_actual, score_crediticio, categoria_riesgo, nivel_vinculo)
    ...
    assert cs_res["score_semilla"] == 60.0  # conocido_referido -> 60 pts
...
def test_registrar_snapshot_atomicidad(db_conn):
    ...
    y actualice atómicamente el perfil del cliente en la tabla clientes.
    """
    cursor = db_conn.cursor()
    cursor.execute("""
        INSERT INTO clientes (nombre, limite_credito, saldo_actual, score_crediticio, categoria_riesgo, nivel_vinculo)
